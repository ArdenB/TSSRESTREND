...
ACP.calculator <- function(CTSR.VI, ACP.table, ACT.table=NULL, allow.negative=FALSE){
  ...
  #Figure out the best regression
  if (dim(mx)[1] <= 2||allow.negative){ # No positve slpoes or allow negative = TRUE
    ...
    if (is.null(ACT.table)){
      suma <- linreg(CTSR.VI, ACP.table[fulname, ], simple = FALSE)
      ...
      suma <- linreg(CTSR.VI, ACP.table[precip.nm, ], ACT.table[part.nm[2],], simple = FALSE)
