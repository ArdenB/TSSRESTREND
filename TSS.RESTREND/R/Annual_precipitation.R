...
AnnualRF.Cal <- function(anu.VI, VI.index, ACP.table, ACT.table=NULL, Breakpoint = FALSE, allow.negative=FALSE){
  ...
  exporter <- function(res.mat, anu.VI, precip.m, temp.m=NULL){
    ...
    if (is.null(temp.m)){
      suma <- linreg(anu.VI, precip.m[fulname, ], simple = FALSE)
      ...
      suma <- linreg(anu.VI, precip.m[precip.nm, ], temp.m[part.nm[2],], simple = FALSE)
      #get a timeseries of the temperature
    ...
    # Return the results
    return(structure(list(summary=suma, annual.precip = anu.ARF, annual.temp = anu.ATM, osp=osp, acp=acp,  tosp=t.osp, tacp=t.acp)))
  }
  ...
  if (!Breakpoint){ # No Breakpoint
    if (allow.negative){ # all values are considered
      ...
      if (dim(mx)[1] <= 2){
        ...
        if (is.null(ACT.table)){ # no temperature data
          ...
          }else{stop("There shold be no tmp data here")}
        # Checked
        return(results)
      }
    }
    ...
    if (allow.negative){ # all values are considered
      ...
      return(structure(list(summary=summ, rf.b4 = results.b4$annual.precip, rf.af= results.af$annual.precip,
                            ...
                            tacp.b4 = results.b4$tacp,osp.af = results.af$osp, acp.af = results.af$acp,
                            tosp.af = results.af$tosp, tacp.af = results.af$tacp)))

    }else{ # has breakpoints, no temperature
      mx <- matrix(m[m[, "slope"] > 0,], ncol=2)
      colnames(mx)<- c("R^2.Value", "slope")
      rownames(mx)<- rownames(m[m[, "slope"] > 0,])
      ...
      if ((dim(mx)[1] <= 2)||(dim(px)[1] <= 2)){
        ...
        results.b4 <- exporter(mx, anu.VI, anu.ACUP[, 1:Breakpoint])
        results.af <- exporter(px, anu.VI, anu.ACUP[, (Breakpoint+1):len])
      }
      #Bind the summary together
      summ <- rbind(results.b4$summary, results.af$summary)
      browser()
